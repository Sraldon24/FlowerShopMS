...
public class SupplierServiceImplTest {
    ...
    void testGetSuppliers() {
        // Mock the repository to return a list of Supplier entities
        when(supplierRepository.findAll()).thenReturn(Arrays.asList(supplierEntity));

        // Mock the mapper to convert each Supplier entity to a SupplierResponseModel
        when(supplierResponseMapper.entityToResponseModel(supplierEntity)).thenReturn(responseModel);
        ...
        assertEquals(1, result.size()); // Ensure the size of the result list is correct
        assertEquals("sup-001", result.get(0).getSupplierId()); // Ensure the supplier ID matches
    }
