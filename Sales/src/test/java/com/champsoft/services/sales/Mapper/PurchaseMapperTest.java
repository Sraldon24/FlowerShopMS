...
public class PurchaseMapperTest {
    ...
    @Test
    void testEntityToResponseModelList_NullInput() {
        PurchaseOrderResponseModelMapper mockResponseMapper = org.mockito.Mockito.mock(PurchaseOrderResponseModelMapper.class);
        org.mockito.Mockito.when(mockResponseMapper.entityToResponseModelList(null)).thenReturn(null);

        List<PurchaseResponseModel> result = mockResponseMapper.entityToResponseModelList(null);
        assertNull(result);
    }

    @Test
    void testEntityToResponseModel_PartialNullIdentifiers() {
        PurchaseOrder order = new PurchaseOrder();
        order.setPurchaseOrderIdentifier(null);
        order.setInventoryIdentifier(new InventoryIdentifier("inv-1001"));
        order.setFlowerIdentifier(null);
        order.setSupplierIdentifier(new SupplierIdentifier("sup-3001"));
        order.setEmployeeIdentifier(null);
        order.setSaleOfferDate(LocalDate.of(2025, 5, 1));
        order.setSalePurchaseStatus(PurchaseStatus.PENDING);
        order.setPrice(new Price(new BigDecimal("49.99"), null)); // currency is null

        PurchaseResponseModel response = responseMapper.entityToResponseModel(order);

        assertNull(response.getPurchaseOrderId());
        assertEquals("inv-1001", response.getInventoryId());
        assertNull(response.getFlowerIdentificationNumber());
        assertEquals("sup-3001", response.getSupplierId());
        assertNull(response.getEmployeeId());
        assertEquals(new BigDecimal("49.99"), response.getSalePrice());
        assertNull(response.getCurrency()); // currency null test
        assertEquals(LocalDate.of(2025, 5, 1), response.getSaleOfferDate());
    }

    @Test
    void testEntityToResponseModel_PriceNull() {
        PurchaseOrder order = new PurchaseOrder();
        order.setPrice(null);
